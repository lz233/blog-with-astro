...
import appDetail1 from './from-vanilla-android-to-flyme/app_detail1.webp';
...
Google 在 Android 14 中对设置的部分界面使用 Compose 进行了重写，其中包含了应用详情界面，而 Flyme 的设置是直接从原生改过来的，只是手动屏蔽了这个 Flag<sup>[1](#appDetail1)</sup>。根据 AOSP 里相关的[代码注释](https://android.googlesource.com/platform/frameworks/base/+/master/core/java/android/util/FeatureFlagUtils.java#254)，可以注意到 `Settings.Global` 中的 Flag 是优先级最高的，覆写即可：`settings put global settings_enable_spa true`。

<ClickableImage id="appDetail1" src={appDetail1} un-lg="w-xl m-auto" />
